...
class BaseCard():
    ...
    def set_field(self, name: str, field: Image.Image | str, *, scale: float | Literal["consistent"] = 1, padding: tuple[int, int] = None, color: tuple[int, int, int] = None) -> bool:
        """
        ...
        if scale == "scale":
            scale = self.scale
        if scale != 1:
            field = self.image.resize((int(scale * field.width), int(scale * field.height)), resample = Image.BOX)
...
im.resize(0.5)
im.set_field("move_description_1", "lmao scaling it\ndown is so\ngoofy", padding=(5, -5), color=(0, 0, 0))
# im.set_field("move_description_2", ".", padding=(5, -5), color=(0, 0, 0))
im.resize(4)
